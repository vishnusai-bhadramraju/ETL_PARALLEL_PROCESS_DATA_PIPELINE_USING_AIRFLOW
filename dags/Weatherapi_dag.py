...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://parallelsbucket/{dt_string}.csv", index=False)
...
with DAG('weather_dag_2',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 'parallelsbucket', 'us_cities.csv', 'us-east-1');"
            )
            ...
            is_houston_weather_api_ready = HttpSensor(
                ...
                endpoint='/data/2.5/weather?q=houston&APPID=17874192a169fd87aa7359ca6913eea2'
            )
